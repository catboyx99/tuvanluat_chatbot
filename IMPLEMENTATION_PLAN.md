...
- **Giai đoạn 14 — Fix regex extract metadata + re-ingest + chống prime ✅ (2026-04-25)**: Probe retrieval cho 46 câu fail target_hit từ run 25/04 cho thấy 78% (36/46) miss vì retrieval không có target trong top-10. Verify ingest: 18/29 unique target_id "không có trong DB" thực ra là **đã ingest nhưng `so_hieu` extract SAI** — 13/18 file Thông tư bị gán số hiệu của Nghị định mà nó hướng dẫn (vd TT 11-2016-BGDĐT bị gán `32/2008/NĐ-CP`, TT 04-2025-BGDĐT bị gán `86/2022/NĐ-CP`...). Nguyên nhân: regex `extract_document_metadata()` trong `document_loader.py` bắt match đầu tiên, mà dòng "Căn cứ Nghị định ..." xuất hiện trước header chính của Thông tư (OCR thường đã làm hỏng dòng "Số: NN/YYYY/..." chính). Fix: thêm `_extract_so_hieu_from_filename()` ưu tiên parse từ tên file (Pattern A: Luật/NĐ/TT/NQ có năm; B: ascii NN_YYYY_TYPE; C: QĐ không có năm; D: file QĐ ascii); fallback `_extract_so_hieu_from_text()` loại bỏ match nằm sau "Căn cứ" (80 chars trước). Helper `_normalize_type()` chuẩn hoá `ND→NĐ`, `QD→QĐ`, `BGDDT→BGDĐT`. Test offline 90 file: 19/19 file fix đúng, 7/7 file gốc không regression, chỉ 1/90 file thực sự không có pattern (`1919_KH-BGDDT_637695.md` — Kế hoạch). Re-ingest `down -v && up -d --build` xong ~12 phút → 89 files / 10276 chunks. Verify lại: tất cả 18 file Bucket A đã có so_hieu chuẩn. Xoá file dup `Thông tư số 30-2023-TT-BGDĐT.md` (giữ `Thông-tư-30-2023-TT-BGDĐT.md`). **Phụ — chống prime lời tư vấn**: phát hiện regression sau Giai đoạn 13: với input ngắn (vd `thanhlapdh`, `quy dinh thanh lap dh`), LLM bị prime bởi câu mẫu `"Thủ tướng Chính phủ quyết định thành lập trường đại học công lập"` trong VD SAI BUG 1 → echo lại nguyên văn câu đó làm toàn bộ lời tư vấn (chỉ 332 chars). Fix: thay tất cả VD SAI BUG 1-4 và VD ĐÚNG bằng placeholder `<NỘI DUNG QUY ĐỊNH>`, `<TÊN VĂN BẢN>`, `<NN/YYYY/TYPE>`... Thêm yêu cầu rõ ràng "lời tư vấn ĐẦY ĐỦ — KHÔNG được rút gọn 1-2 câu, phải có (a) trả lời trực tiếp + (b) liệt kê điều kiện/thẩm quyền/quy trình + (c) chia mục bằng heading/bullet". Verify: 4 câu test cũ trở lại 2166-2963c (trước 332-1600c).

**Phụ 2 — Force cite Luật gốc + harden post-process**: phát hiện thêm bug khi user hỏi "thanhlapdaihoc" / "thành lập đh" / "thanh lap dai hoc": LLM bỏ Luật `08/2012/QH13` (rank 8 trong top-10) chỉ cite NĐ `125/2024/NĐ-CP` (rank 1-7,9). Probe xác nhận retrieval CÓ Luật, LLM ngó lơ. Thêm 3 fix: (1) RULE BẮT BUỘC #1 ở đầu prompt: "scan toàn bộ context tìm chunk có `[Meta: Số hiệu: NN/YYYY/QHxx]`, BẮT BUỘC cite Luật đó ở vị trí ĐẦU TIÊN, kể cả khi chỉ có 1/10 chunk"; (2) `CITATION_MARKER_RE` linh hoạt match `**`/`##`/`#`/`bare` thay vì chỉ `**Căn cứ pháp lý` (LLM hay xuất `## Căn cứ pháp lý` heading H2); luôn normalize header về `**Căn cứ pháp lý:**` ở output; (3) thêm `CITE_LINE_NOPAREN_RE` fallback bắt format LLM hay xuất thiếu paren ("Nghị định 125/2024/NĐ-CP, ..."); (4) khi dòng BUG 2 (thiếu tên) là dòng duy nhất cho 1 số hiệu — suy ra tên generic từ type suffix (`/QHxx`→"Luật", `/NĐ-CP`→"Nghị định", `/TT-…`→"Thông tư"...) thay vì drop. Verify 3 câu thật: tất cả cite đầy đủ Luật + NĐ, format chuẩn. User tự re-run test_100_questions_4lite để đo target_hit mới (kỳ vọng BHYT 16.7%→~50%, Giáo dục 43.9%→~65%).
...
### Giai đoạn 14 — Fix regex extract metadata + re-ingest ✅
**Bối cảnh**: Probe retrieval 25/04 (`auto_test_serverside/probe_retrieval.py` + `verify_by_source.py`) phát hiện 13/18 file Thông tư đang bị gán số hiệu của Nghị định khác (vd TT 11-2016-BGDĐT bị gán `32/2008/NĐ-CP`, TT 04-2025-BGDĐT bị gán `86/2022/NĐ-CP`...) — root cause của target_hit thấp ở Bucket B (36/46 câu fail). Nguyên nhân: regex `extract_document_metadata()` bắt match đầu tiên trong file, mà dòng "Căn cứ Nghị định ..." xuất hiện trước header chính của Thông tư.

**Các bước**:
- [x] B1. Đọc `extract_document_metadata()` — phát hiện regex bắt match đầu tiên trong `head[:5000]`, không loại trừ "Căn cứ"
- [x] B1. Thêm `_extract_so_hieu_from_filename()` (4 pattern: Luật/NĐ/TT/NQ có năm; ascii NN_YYYY_TYPE; QĐ có/không năm; QĐ ascii); `_extract_so_hieu_from_text()` loại bỏ match sau "căn cứ" trong 80 chars; helper `_normalize_type()` chuẩn hoá `ND→NĐ`, `QD→QĐ`, `BGDDT→BGDĐT`
- [x] B1. Test offline trên 90 file (`auto_test_serverside/test_extract_offline.py`): 19/19 file Bucket A fix đúng, 7/7 file gốc không regression, 1/90 file `1919_KH-BGDDT_637695.md` (Kế hoạch — không pattern chuẩn) bỏ
- [x] B2. Re-ingest: `docker compose down -v && docker compose up -d --build` (~12 phút, 89 files → 10276 chunks)
- [x] B2. Verify: `verify_by_source.py` xác nhận tất cả 18 file Bucket A đã có so_hieu chuẩn
- [x] B3. Xoá file dup `md_materials/Thông tư số 30-2023-TT-BGDĐT.md`
- [x] B5. (Phụ) Chống prime lời tư vấn: sau Giai đoạn 13 phát hiện input ngắn (`thanhlapdh`, `quy dinh thanh lap dh`) bị LLM rút gọn lời tư vấn xuống 332c — nguyên nhân do echo câu mẫu `"Thủ tướng Chính phủ quyết định thành lập trường đại học công lập"` trong VD SAI BUG 1
- [x] B5. Thay VD SAI BUG 1-4 + VD ĐÚNG bằng placeholder `<NỘI DUNG QUY ĐỊNH>`, `<TÊN VĂN BẢN>`, `<NN/YYYY/TYPE>` — không còn câu mẫu cụ thể để LLM bám vào
- [x] B5. Thêm yêu cầu "lời tư vấn ĐẦY ĐỦ — KHÔNG rút gọn 1-2 câu" với 3 cấu phần bắt buộc (a/b/c)
- [x] B5. Verify offline: 4 câu test bị rút gọn trước đó hồi phục về 2166-2963c (commit `65a25ff`)
- [x] B5b. Phát hiện bug: hỏi `thanhlapdaihoc`/`thành lập đh` → LLM bỏ Luật `08/2012/QH13` chỉ cite NĐ. Probe xác nhận retrieval CÓ Luật ở rank 8 → đây là Bucket A "LLM ngó lơ"
- [x] B5b. Prompt: thêm "RULE BẮT BUỘC #1" — bắt buộc cite mọi `/QHxx` trong context ở vị trí đầu, kể cả khi chỉ có 1/10 chunk
- [x] B5b. Post-process: `CITATION_MARKER_RE` linh hoạt (chấp nhận `**`/`##`/`#` prefix) + luôn normalize header về `**Căn cứ pháp lý:**`
- [x] B5b. Post-process: thêm `CITE_LINE_NOPAREN_RE` bắt format thiếu paren ("Nghị định 125/2024/NĐ-CP, ...")
- [x] B5b. Post-process: dòng BUG 2 đơn lẻ → suy ra tên generic từ type suffix (`/QHxx`→"Luật", `/NĐ-CP`→"Nghị định"...) thay vì drop
- [x] B5b. Verify 3 câu test: tất cả có Luật + NĐ, format chuẩn (commit `46b0019`)
- [ ] B6. **Chờ user deploy server** (`/deploy` skill) — pull code mới + rebuild trên 113.161.95.116, server tự re-ingest từ md_materials
- [ ] B7. **Chờ user re-run `test_100_questions_4lite.py` trên server** — đo target_hit mới (kỳ vọng BHYT ≥50%, Giáo dục ≥65%, tổng ≥60%) + verify lời tư vấn không bị rút gọn ở các câu input ngắn
- [ ] B7. Cập nhật Section 2.5 Giai đoạn 14 với số liệu thực tế sau test