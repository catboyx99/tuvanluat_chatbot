...
- [ ] Commit fix tên Luật + push → deploy server **Mode B** (`docker compose down -v && docker compose up -d --build` do md_materials/* thay đổi). Verify "thành lập đại học" hiển thị `Luật Giáo dục đại học (Luật số 08/2012/QH13), ban hành ngày 18/06/2012, Điều 27`.
- [ ] Re-run `test_100_questions_4lite.py` trên server → đo target_hit mới (kỳ vọng BHYT ≥50%, Giáo dục ≥65%, tổng ≥60%) + verify lời tư vấn không rút gọn. Cập nhật Section 2.5 với số liệu thực tế.